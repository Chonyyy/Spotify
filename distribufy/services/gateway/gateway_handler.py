...
class GatewayRequestHandler(ChordNodeRequestHandler):
    def do_POST(self):
        ...
        if self.path == '/gw/store-song-file':#TODO
            ...
            song_key = self.post_data.get('song_title')
            udp_ip = self.post_data.get('client_ip')
            udp_port = int(self.post_data.get('client_port'))
            ...
            if song_key and udp_ip and udp_port is not None:
                ...
                # logger.debug(f'returning success to client: {success}')
                self.send_json_response(success)  # Return the UDP IP and port
                # if success:
